//! ALSA PCM mmap output state.
//!
//! This module owns the raw `snd_pcm_*` FFI declarations plus the
//! `AlsaMmapCtx` wrapper that opens a device in `MMAP_INTERLEAVED` mode and
//! commits frames in zero-copy fashion. It is shared between the existing
//! GStreamer-fed mmap writer thread (in `lib.rs`) and the V2 native_transport
//! ALSA output backend.
